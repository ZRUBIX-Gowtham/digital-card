...
const content = `import type { CardData } from "@/types/card";
import {
...
export function renderVideo(card: CardData, accent: string, opts?: SignatureStyleOpts): string {
  ...
  // YouTube Video Block
  const ytUrl = (card.youtubeVideos && card.youtubeVideos.length > 0) ? card.youtubeVideos[0] : "https://youtube.com";
  ...
  const videoTitle = opts?.bannerText || "Check out my latest video!";
...
fs.writeFileSync('src/lib/signatures/video.ts', content);
console.log('Fixed video.ts string escaping');
